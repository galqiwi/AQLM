...
class ADMMTest(unittest.TestCase):
    def test_reference(self):
        in_size, out_size, n_samples = 128, 256, 1024
        X = torch.randn(n_samples, in_size)
        XTX = X.T @ X
        target = torch.randn(out_size, in_size)

        output_ref = admm_reference(target, XTX, 0.5)
        output = admm_prune(target, XTX, 0.5)

        torch.testing.assert_allclose(actual=(output == 0.), expected=(output_ref == 0.))
        torch.testing.assert_allclose(actual=output, expected=output_ref)
