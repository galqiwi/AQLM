...
def main():
    ...
    for param_name, param_value in tqdm(best_model_state_dict.items()):
        if param_name.endswith(SCALES_INPUT_SUFFIX):
            ...
            hf_aqlm_model_state_dict[f'{prefix}{SCALES_OUTPUT_SUFFIX}'] = param_value.to(torch.float16)
            continue

        if param_name.endswith(CODEBOOKS_INPUT_SUFFIX):
            prefix = param_name[:-len(CODEBOOKS_INPUT_SUFFIX)]

            hf_aqlm_model_state_dict[f'{prefix}{CODEBOOKS_OUTPUT_SUFFIX}'] = param_value.to(torch.float16)
            continue
        ...
        if param_name.endswith(CODES_INPUT_SUFFIX):
            ...
            hf_aqlm_model_state_dict[f'{prefix}{CODES_OUTPUT_SUFFIX}'] = codes()
            continue

        hf_aqlm_model_state_dict[param_name] = param_value
    ...
    for module_name, module in base_model.named_modules():
        ...
        for param_name, param in module.named_parameters():
            linear_weights_not_to_quantize.append(f'{module_name}.{param_name}')
